...
describe('user authentication actions login', () => {
  ...
  it('should call the login success dispatch function', async () => {
    axios.fetchPopularArticles = jest.fn().mockResolvedValue(fetchArticlesresponse);
  ...
  it('should call the login success dispatch function', async () => {
    axios.fetchTrendingArticles = jest.fn().mockResolvedValue(fetchArticlesresponse);
    await actions.fetchtrending()(dispatch);
    expect(dispatch).toBeCalledTimes(1);
  });
  ...
  it('should throw error', async () => {
    axios.fetchTrendingArticles = jest.fn(() => {
      throw { response: 'hello' };
    });
    try {
      await actions.fetchtrending(payload)(dispatch);
    } catch (error) {
      expect(error).toEqual({ response: 'hello' });
    }
    expect(dispatch).toBeCalledTimes(1);
